...
export class GetHomeData {
    async execute(dto) {
        ...
        const todayWorkoutDay = activeWorkoutPlan.workoutDays.find((workoutDay) => workoutDay.weekDay === targetWeekDay);
        if (!todayWorkoutDay) {
        ...
        const completedSessionDates = new Set(completedSessions.map((session) => `${session.workoutDayId}:${dayjs.utc(session.startedAt).format("YYYY-MM-DD")}`));
        const workoutDayByWeekDay = new Map(activeWorkoutPlan.workoutDays.map((workoutDay) => [workoutDay.weekDay, workoutDay]));
        let workoutStreak = 0;
